...
const qualities = ["a fantasic", "a great", "a mediocre", "an okay", "a terrible", "an awful", "a boring", "a fun"];
...
const createMessage = () = => {
    return `${games[randomNum(games.length)]} is ${qualities[randomNum(qualities.length)]} game. ${recommendations[randomNum(recommendations.length)]} I'd give it a solid ${scores[randomNum(scores.length)]}.`
};
